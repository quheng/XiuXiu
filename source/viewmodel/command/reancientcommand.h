...
template <class T>
class ReancientCommand :public ICommandEx
{
public:
    ReancientCommand(T* p) throw():m_pviemod(p)
    {
    }

    RefPtr<ICommandEx> getReancientCommandRef() throw()
    {
         return RefPtrHelper::TypeCast<ReancientCommand<T>, ICommandEx>(RefPtr<ReancientCommand<T> >(this));
    }
    ...
    {
        //改变model层数据
        m_pviemod->getImageModelRef().Deref().DoReancient();
