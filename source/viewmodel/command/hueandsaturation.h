...
template <class T>
class HueSaturaCommand :public ICommandEx
{
public:
    HueSaturaCommand(T* p) throw():m_pviemod(p)
    {
    }

    RefPtr<ICommandEx> getHueSaturaCommandRef() throw()
    {
         return RefPtrHelper::TypeCast<HueSaturaCommand<T>, ICommandEx>(RefPtr<HueSaturaCommand<T> >(this));
    }
    ...
    virtual void Exec()
    {
        ...
        //改变model层的数据
        m_pviemod->getImageModelRef().Deref().AdHueAndSaturation();
